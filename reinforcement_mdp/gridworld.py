        ...
        if not opts.manual:
            if opts.valueSteps:
                ...
                for i in range(opts.iters):
                    if opts.agent in ('value', 'asynchvalue', 'priosweepvalue'):
                        tempAgent = valueIterationAgents.ValueIterationAgent(mdp, opts.discount, i)
                    elif opts.agent == 'rtdp':
                        tempAgent = rtdpAgents.RTDPAgent(mdp, opts.discount, i)
                    else:
                        raise Exception("Agent type {} not supported.".format(opts.agent))
                    display.displayValues(tempAgent, message = "VALUES AFTER {} ITERATIONS".format(i))
                    display.pause()

            display.displayValues(a, message = "VALUES AFTER {} ITERATIONS".format(opts.iters))
            display.pause()
            display.displayQValues(a, message = "Q-VALUES AFTER {} ITERATIONS".format(opts.iters))
            display.pause()
    except KeyboardInterrupt:
        sys.exit(0)
